...
void init(){ active.clear(), active.resize(n+1); }
bool cmp(Point p1, Point p2){
	ll f1 = p1.nume*p2.denom;
	ll f2 = p1.denom*p2.nume;
	if (f1 == f2) return p1.typ < p2.typ;
	return f1 < f2;
}
...
void angularSort(){
	sort(ALL(q1),cmp), sort(ALL(q2),cmp);
	sort(ALL(q3),cmp), sort(ALL(q4),cmp);
	for(auto elm : q1) a.pb(elm);
	for(auto elm : q2) a.pb(elm);
	for(auto elm : q3) a.pb(elm);
	for(auto elm : q4) a.pb(elm);
}
void angularSweep(){ // go through the max amount of points from the origin [0,0]
	ll pos = 0,cnt=0,cur=0,ans=0;
...
void lineSettings(){
	...
	fore(i,0,n){
		...
		p1.nume = p1.y/g1, p1.denom = p1.x/g1;
		p2.nume = p2.y/g2, p2.denom = p2.x/g2;
		...
		if (p1.quad == 1 && p2.quad == 3){
			...
			if (xInt > 0.0) p2.typ = 1, p1.typ = 2;
			else p1.typ = 1, p2.typ = 2;
		}
		...
		toqVector(p1), toqVector(p2);
	}
}
void doit(){
	lineSettings(), angularSort(), angularSweep();
}