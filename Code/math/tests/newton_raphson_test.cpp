...
// Main: returns all real roots (unique, sorted)
...
int main() {
    ...
    while (cin >> N) {
        ...
        // Input order: aN aN-1 ... a0
        // Our storage: a[0]..a[N]
        vector<ld> a(N + 1);
        for (int i = N; i >= 0; --i) cin >> a[i];

        auto roots = all_real_roots(a);

        // The problem says there are N real distinct roots.
        // In case of tiny numeric issues, you can keep first N after sorting:
        sort(roots.begin(), roots.end());
        if ((int)roots.size() > N) roots.resize(N);
