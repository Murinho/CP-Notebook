...
void doit(){
	...
	for (int i = 0; i < q; i++) {
		...
		if (t == "link") {
            /*
            Add an edge from vertex A to B, making A a child of B, where initially A is a
            root vertex, A and B are in different trees.
            */
			int u, v;
			cin >> u >> v;
			u--, v--;
			link(lct[v], lct[u], 1);
		} else if (t == "cut") { // remove node from parent
			int u;
			cin >> u;
			u--;
			cut(lct[u]);
		} else if (t == "lca") { // compute lca between 2 nodes (returns -1 if not found)
			int u, v;
			cin >> u >> v;
			u--, v--;
			cout << lca(lct[u], lct[v])->val + 1 << nl;
		} else if (t == "add_val"){ // add a value to a single node.
			int p, x;
			cin >> p >> x;
			p--;
			lct[p]->access();
			lct[p]->val += x;
			lct[p]->calc();
		} else if (t == "path_sum"){ // compute path sum between u and v.
			int u, v;
			cin >> u >> v;
			u--, v--;
			lct[u]->makeRoot();
			lct[v]->access();
			cout << lct[v]->sum << nl;
		} else if (t == "conn"){ // check if two nodes are connected.
			int u,v;
			cin >> u >> v;
			u--, v--;
			cout << (connected(lct[u],lct[v]) ? "YES" : "NO") << nl;
		} else if (t == "rem"){ // remove two nodes.
			int u,v;
			cin >> u >> v;
			u--, v--;
			cut(lct[u],lct[v]);
		} else if (t == "sum_subtree"){
			/*
			For an edge (v,p) where p is considered the parent,
			output the sum of weights of all vertices in the
			subtree of v.
			*/
			int v, p;
			cin >> v >> p;
