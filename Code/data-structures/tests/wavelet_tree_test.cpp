// Tested with AC: https://cses.fi/problemset/task/3163/
#include <bits/stdc++.h>
...
struct WaveletTree {
  ...
  int count_comp(int l, int r, int x, int y) const {
    ...
    int lL = lb + 1, rL = rb;
    int lR = (l - lb), rR = (r - rb);
    return (L ? L->count_comp(lL, rL, x, y) : 0)
         + (R ? R->count_comp(lR, rR, x, y) : 0);
  }
  ...
  int kth_comp(int l, int r, int k) const {
    ...
    if (lo == hi) return lo;
    int lb = mapLeft[l - 1], rb = mapLeft[r];
    int inLeft = rb - lb;                // how many in left child
    if (k <= inLeft) {
      return L->kth_comp(lb + 1, rb, k);
    } else {
...
struct WaveletIndex {
  ...
  explicit WaveletIndex(const vi &arr) {
    ...
    for (int i = 0; i < sz(a); i++) {
      a_comp[i] = int(lower_bound(ALL(compVals), a[i]) - compVals.begin()) + 1; // 1..σ
    }
  ...
  int compress_floor_idx(int v) const {
    ...
    return int(it - compVals.begin());           // 0..σ
  }
  int compress_ceil_idx(int v) const {
    auto it = lower_bound(ALL(compVals), v);
    if (it == compVals.end()) return (int)compVals.size() + 1; // σ+1
    return int((it - compVals.begin()) + 1);    // 1..σ+1
  }
  ...
  int kth_smallest(int l, int r, int k) const {
    ...
    int len = r - l + 1;
    if (l > r || k < 1 || k > len) {
      // choose behavior: throw, return sentinel, etc.
      // Here we return INT_MIN as sentinel.
      return INT_MIN;
    }
  ...
  int kth_largest(int l, int r, int k) const {
    ...
    // largest ↔ smallest with k' = len - k + 1
